...
#include "NCTrackedUser.h"

// CALLBACKS
// =============================================================================
// Callback: New user was detected
void XN_CALLBACK_TYPE NewUser_detected(
                                   ...
                                   )
{
    ...
    NCUserGenerator* user = static_cast<NCUserGenerator*>(pCookie);
    if(user->needsPoseForCalibration()) {
     ...
     int i = nID;
     ofNotifyEvent(user->newUser,i,user);
}

// Callback: An existing user was lost
void XN_CALLBACK_TYPE LostUser_detected(
                                    xn::UserGenerator& rGenerator
                                    ,XnUserID nID
                                    ,void* pCookie
                                    )
{
    ...
    NCUserGenerator* user = static_cast<NCUserGenerator*>(pCookie);
    int i = nID;
    ofNotifyEvent(user->lostUser,i,user);
...
// OFXUSERGENERATOR
// =============================================================================
NCUserGenerator::NCUserGenerator() {
    needs_pose = false;
...
//----------------------------------------
void NCUserGenerator::startPoseDetection(XnUserID nID) {
...
//----------------------------------------
void NCUserGenerator::stopPoseDetection(XnUserID nID) {
    user_generator.GetPoseDetectionCap().StopPoseDetection(nID);
}


//----------------------------------------
void NCUserGenerator::requestCalibration(XnUserID nID) {
...
//----------------------------------------
bool NCUserGenerator::setup( ofxOpenNIContext* pContext) {
    ...
    for(int i = 0; i < MAX_NUMBER_USERS; ++i) {
        ...
        NCTrackedUser* tracked_user = new NCTrackedUser(context);
...
//----------------------------------------
void NCUserGenerator::drawUser(int nUserNum, const float wScale, const float hScale) {
...
//----------------------------------------
void NCUserGenerator::draw(const int width, const int height) {
...
// Get a tracked user.
//----------------------------------------
NCTrackedUser* NCUserGenerator::getTrackedUser(int nUserNum) {
...
void NCUserGenerator::setMaxNumberOfUsers(int nUsers) {
...
int NCUserGenerator::getNumberOfTrackedUsers() {
    return found_users;
}

// Update the tracked users, should be called each frame
//----------------------------------------
void NCUserGenerator::update() {
...
void NCUserGenerator::setUseMaskPixels(bool b) {
    useMaskPixels = b;
}

void NCUserGenerator::setUseCloudPoints(bool b) {
    useCloudPoints = b;
}
...
//----------------------------------------
unsigned char * NCUserGenerator::getUserPixels(int userID) {
...
// or specify a number if you want seperate masks
//----------------------------------------
void NCUserGenerator::updateUserPixels() {
...
//----------------------------------------
void NCUserGenerator::updateCloudPoints() {
...
ofPoint NCUserGenerator::getWorldCoordinateAt(int x, int y, int userID) {
    
    return cloudPoints[userID][y * width + x];
    
}

ofColor NCUserGenerator::getWorldColorAt(int x, int y, int userID) {
...
//--------------------------------------------------------------
void NCUserGenerator::setSmoothing(float smooth) {
...
//--------------------------------------------------------------
float NCUserGenerator::getSmoothing() {
    return smoothing_factor;
}

//----------------------------------------
int NCUserGenerator::getWidth() {
    return width;
}

//----------------------------------------
int NCUserGenerator::getHeight() {
    return height;
}

//----------------------------------------
void NCUserGenerator::startTracking(XnUserID nID) {
    user_generator.GetSkeletonCap().StartTracking(nID);
}

//----------------------------------------
bool NCUserGenerator::needsPoseForCalibration() {
    return needs_pose;
}

// Get a ref to the xn::UserGenerator object.
//----------------------------------------
xn::UserGenerator& NCUserGenerator::getXnUserGenerator() {
